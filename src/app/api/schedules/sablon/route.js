...
async function isSupplierAvailable(supplierId, startDate, endDate, excludeId = null) {
  let sql = `
    SELECT COUNT(*) as count FROM schedule_sablon 
    WHERE supplier_id = ? 
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      message: 'Schedule sablon berhasil ditambahkan',
...
export async function PUT(request) {
  try {
    ...
    return NextResponse.json({ message: 'Schedule sablon berhasil diupdate' });
  } catch (error) {
...
export async function DELETE(request) {
  try {
    ...
    await query('DELETE FROM schedule_sablon WHERE id = ?', [id]);

    return NextResponse.json({ message: 'Schedule sablon berhasil dihapus' });
  } catch (error) {
