...
async function isSupplierAvailable(supplierId, startDate, endDate, excludeId = null) {
  let sql = `
    SELECT COUNT(*) as count FROM schedule_jahit 
    WHERE supplier_id = ? 
...
export async function DELETE(request) {
  try {
    ...
    await query('DELETE FROM schedule_jahit WHERE id = ?', [id]);
